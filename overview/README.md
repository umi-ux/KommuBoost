...
On top of that, a hardware-level analog switch defaults to pass-through when the MCUs' control signals are absent, this covers MCU or firmware failure while the board still has power. It does not cover total loss of board power, that scenario currently results in no signal reaching the ECU at all, not a safe pass-through. This is a real, unresolved gap in the design, see [Validation Requirements](../open-items/#critical-total-power-loss-is-not-currently-covered-by-any-fail-safe) for detail before treating the power-loss story as settled.
