 ...
 * Anything not explicitly confirmed defaults toward the STRICTER
 * bucket - flag with Ting before treating this table as locked, since
 * it IS the FMEDA-relevant decision.
 */
...
static inline fault_class_t fault_classify(fault_code_t f)
{
    ...
    /* --- NOT yet confirmed - defaulting to stricter bucket on purpose,
     * flag each with Ting rather than assume: */
