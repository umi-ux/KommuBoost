...
| Supervisor MCU | `main_supervisor.h/.c` | Holds physical veto via `FORCE_PT`/`GATE_ENABLE`, independent monitoring |
...
- **Supervisor owns final veto authority** via `FORCE_PT` and `GATE_ENABLE`. The main MCU, despite owning the state machine, does not have the final word on whether boost/pass-through is engaged. See [Key Learnings](../key-learnings/README.md#supervisor-holds-veto-is-correct-for-asil-b) for the safety rationale.
